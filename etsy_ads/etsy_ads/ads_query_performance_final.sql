...
where
	date(datetime(_PARTITIONTIME,"America/New_York")) = active_date
)
;
...
from
	query_prolist_imps a left join
...
CREATE TEMPORARY TABLE
  ...
  as (
	select
		...
		b.power_shop_status
	from
		click_purchase_merge a left join
		prolist_listing_history b on cast(a.listing_id as int64) = b.listing_id
)
;


CREATE TEMPORARY TABLE
  query_page_listing_history
  as (
select
	event_date_est,
	query,
...
from
	...
	`etsy-data-warehouse-prod.weblog.recent_visits` b on a.visit_id = b.visit_id and b._date >= active_date - 1 and date(datetime(b.start_datetime,"America/New_York")) = active_date
where
	a._date >= active_date - 1
...
from
	...
	`etsy-data-warehouse-prod.weblog.recent_visits` b on a.visit_id = b.visit_id and b._date >= current_date - 3 and date(datetime(b.start_datetime,"America/New_York")) = active_date
where
	a._date >= active_date - 1
...
select
  	...
  	-- overall search metrics
		...
		query_group_label,
		query_group_label_conf_score,
		row_number() over(partition by event_date_est order by total_ads_queries desc) as query_rank
from
	query_clicks_purchases a left join
	query_sessions_empty b on a.query = b.query and a.event_date_est = b.event_date_est and a.page_type_group = b.page_type_group left join
	query_page_listing_history c on a.query = c.query and a.event_date_est = c.event_date_est and a.page_type_group = c.page_type_group left join
	search_hour_chunk_data d on a.query = d.query and a.event_date_est = d.event_date_est and a.page_type_group = d.page_type_group left join
	query_level_data e on a.query = e.query
)
;



-- create a daily/hourly rollup with summary stats. similar to prior table but removing the query
-- as a grouped dimension
CREATE TABLE IF NOT EXISTS
  `etsy-data-warehouse-dev.rollups.etsy_ads_hourly_performance`
  (
  	event_date_est DATE NOT NULL,
  	event_hour_est INT64,
  	page_type_group STRING,
		total_ads_queries INT64,
		total_ads_empty_queries INT64,
		total_ads_result_count INT64,
...
DELETE FROM `etsy-data-warehouse-dev.rollups.etsy_ads_hourly_performance` where event_date_est = active_date
...
INSERT INTO `etsy-data-warehouse-dev.rollups.etsy_ads_hourly_performance` (
select
	a.event_date_est,
	a.event_hour_est,
	a.page_type_group,
  ...
  distinct_listing_ads_imp,
	...
	total_search_purchases
from
	hourly_ads_clicks a left join
	hourly_query_sessions_empty b on a.event_date_est = b.event_date_est and a.event_hour_est = b.event_hour_est and a.page_type_group = b.page_type_group left join
	hourly_listing_history c on a.event_date_est = c.event_date_est and a.event_hour_est = c.event_hour_est and a.page_type_group = c.page_type_group left join
	hourly_search_data d on a.event_date_est = d.event_date_est and a.event_hour_est = d.event_hour_est and a.page_type_group = d.page_type_group
)
PARTITION BY event_date_est;
